(function(){
  ...
  async function gerar(opts={}){
    ...
    const responsavel=campo('orcResponsavel')||'—',contato=campo('orcContatoResponsavel')||'—',aluno=campo('orcAluno')||'—',nasc=campo('orcNascimento'),obs=campo('orcObservacoes');
    ...
    y=72;doc.setTextColor(...azul);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('DADOS DO ORÇAMENTO',M,y);y+=6;doc.setFillColor(...claro);doc.roundedRect(M,y,L-2*M,55,2,2,'F');doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(35,55,75);
    const i=idade(nasc);
    const linhas=[['Nº do orçamento:',id,'Data/Hora:',dataHora],['Aluno:',aluno,'Responsável:',responsavel],['Contato:',contato,'Nascimento:',dataBR(nasc)],['Idade:',i!==''?i+' anos':'—','Série/Turma:',c.s.nome||'—'],['Condição:',cond,'Plano:',plano],['STI:',c.stiModalidade||'Sem STI','Produto STI:',c.stiProduto||'—'],['Referência:','Valores oficiais 2027','','']];
    ...
    y+=63;doc.setTextColor(...azul);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('CONDIÇÕES DO PLANO',M,y);y+=5;doc.setFillColor(245,249,253);doc.roundedRect(M,y,L-2*M,25,2,2,'F');doc.setTextColor(35,55,75);doc.setFontSize(9);
    ...
    const itens=[];if(c.includeAnnual)itens.push(['Anuidade',1,c.anuidade,c.anuidade]);if(c.includeFirst&&!c.includeAnnual)itens.push(['1ª parcela',1,c.primeira,c.primeira]);if(c.includeMat)itens.push(['Livros / material didático',1,c.material,c.material]);(c.uniformes||[]).forEach(u=>itens.push([u.item,u.qt,u.unit,u.sub]));(c.sti||[]).forEach(u=>itens.push([u.item,u.qt,u.unit,u.sub]));
    ...
    if((c.sti||[]).length){y+=4;if(y>238){doc.addPage();y=18}doc.setFillColor(245,249,253);doc.roundedRect(M,y,L-2*M,20,2,2,'F');doc.setTextColor(...azul);doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('STI / TEMPO INTEGRAL',M+5,y+6);doc.setFont('helvetica','normal');doc.setTextColor(35,55,75);doc.text(`Modalidade: ${c.stiModalidade||'—'}`,M+5,y+12);doc.text(`Valor total STI: ${moeda(c.stiTotal||0)}`,M+5,y+17);if(c.stiParcelamento)doc.text(String(c.stiParcelamento),M+78,y+12,{maxWidth:92});y+=25}
