(function(){
  ...
  async function gerar(opts={}){
    ...
    y=72;doc.setTextColor(...azul);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('DADOS DO ORÇAMENTO',M,y);y+=6;doc.setFillColor(...claro);doc.roundedRect(M,y,L-2*M,48,2,2,'F');doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setTextColor(35,55,75);
    const i=idade(nasc);
    const linhas=[['Nº do orçamento:',id,'Data/Hora:',dataHora],['Aluno:',aluno,'Responsável:',responsavel],['Nascimento:',dataBR(nasc),'Idade:',i!==''?i+' anos':'—'],['Série/Turma:',c.s.nome||'—','Condição:',cond],['Plano:',plano,'STI:',c.stiModalidade||'Sem STI'],['Produto STI:',c.stiProduto||'—','Referência:','Valores oficiais 2027']];
    ...
    y+=56;doc.setTextColor(...azul);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('CONDIÇÕES DO PLANO',M,y);y+=5;doc.setFillColor(245,249,253);doc.roundedRect(M,y,L-2*M,25,2,2,'F');doc.setTextColor(35,55,75);doc.setFontSize(9);
