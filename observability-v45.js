(function(){
  ...
  function push(type,payload){
    ...
    try{sessionStorage.setItem('cora_observability_buffer',JSON.stringify(buffer))}catch(e){}
    return item;
  }
  function loadSentry(){
    ...
    return new Promise(resolve=>{
      ...
      s.onload=()=>{try{window.Sentry?.init({dsn:cfg.sentryDsn,environment:cfg.environment||'production',release:cfg.release||'cora-familia-web',tracesSampleRate:Number(cfg.tracesSampleRate??0.05),sendDefaultPii:false});resolve(window.Sentry||null)}catch(e){resolve(null)}};
  ...
  function captureError(error,context={}){
    ...
    try{if(window.Sentry?.captureException)window.Sentry.captureException(error instanceof Error?error:new Error(message),{extra:context})}catch(e){}
  }
