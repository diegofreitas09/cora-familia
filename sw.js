const C='cora-familia-v37';
const A=['./','./index.html','./app.css','./app.js','./auth.js','./config.js','./data.js','./cora-gestao-sync-v27.js','./orcamento-primeira-fix-v29.js','./orcamentos-registro-v30.js','./orcamento-verificacao-v35.js','./pdf-download.js','./pdf-unificado-v37.js','./orcamento-save-v37.js','./manifest.webmanifest','./logo-escola.png','./icon-192.png','./icon-512.png','./apple-touch-icon.png','./fachada-cora-familia.jpg'];
...
self.addEventListener('fetch',e=>{
 ...
 if(url.origin===self.location.origin&&(url.pathname.endsWith('/cora-familia/')||url.pathname.endsWith('/cora-familia/index.html'))){
  e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{let html=await r.text();
   html=html.replace(/cora-gestao-sync-v27\.js(?:\?v=\d+)?/g,'cora-gestao-sync-v27.js?v=3').replace(/orcamento-primeira-fix-v29\.js(?:\?v=\d+)?/g,'orcamento-primeira-fix-v29.js?v=1').replace(/orcamentos-registro-v30\.js(?:\?v=\d+)?/g,'orcamentos-registro-v30.js?v=4').replace(/orcamento-verificacao-v35\.js(?:\?v=\d+)?/g,'orcamento-verificacao-v35.js?v=1').replace(/pdf-download\.js(?:\?v=\d+)?/g,'pdf-download.js?v=4');
   ...
   if(!html.includes('pdf-download.js'))html=html.replace('</body>',"<script src='pdf-download.js?v=4'></script></body>");
   if(!html.includes('pdf-unificado-v37.js'))html=html.replace('</body>',"<script src='pdf-unificado-v37.js?v=1'></script></body>");
   if(!html.includes('orcamento-save-v37.js'))html=html.replace('</body>',"<script src='orcamento-save-v37.js?v=1'></script></body>");
