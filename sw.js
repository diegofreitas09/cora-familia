...
self.addEventListener('fetch',e=>{
  ...
  if(url.origin===self.location.origin&&(url.pathname.endsWith('/cora-familia/')||url.pathname.endsWith('/cora-familia/index.html'))){
    e.respondWith(
      fetch(e.request,{cache:'no-store'}).then(async r=>{
        ...
        if(!html.includes('orcamentos-registro-v30.js')) html=html.replace('</body>',"<script src='orcamentos-registro-v30.js?v=1'></script></body>");
        else html=html.replace(/orcamentos-registro-v30\.js(?:\?v=\d+)?/g,'orcamentos-registro-v30.js?v=1');
  ...
  if(url.origin===self.location.origin&&['/app.js','/data.js','/cora-gestao-sync-v27.js','/orcamento-primeira-fix-v29.js','/orcamentos-registro-v30.js'].some(x=>url.pathname.endsWith(x))){
