const C='cora-familia-v26';
const A=['./','./index.html','./app.css','./app.js','./auth.js','./config.js','./data.js','./pdf-download.js','./manifest.webmanifest','./logo-escola.png','./icon-192.png','./icon-512.png','./apple-touch-icon.png','./fachada-cora-familia.jpg'];
  ...
  if(url.origin===self.location.origin&&(url.pathname.endsWith('/cora-familia/')||url.pathname.endsWith('/cora-familia/index.html'))){
    e.respondWith(
      fetch(e.request).then(async r=>{
        ...
        if(!html.includes('pdf-download.js')) html=html.replace('</body>',"<script src='pdf-download.js?v=1'></script></body>");
