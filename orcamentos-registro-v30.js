(function(){
  ...
  async function save(){
    const responsavel=(q('orcResponsavel')?.value||'').trim(),contato=(q('orcContatoResponsavel')?.value||'').trim(),aluno=(q('orcAluno')?.value||'').trim(),nasc=q('orcNascimento')?.value||'',birth=parseBirth(nasc),idade=ageFromBirth(nasc),observacoes=(q('orcObservacoes')?.value||'').trim(),status=q('orcSaveStatus'),btn=q('saveQuote');
    if(!responsavel||!contato||!aluno||!nasc){status.className='orc-status err';status.textContent='Preencha nome e contato do responsável, nome do aluno e data de nascimento.';return}
