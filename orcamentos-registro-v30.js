(function(){
  ...
  async function save(){
    const responsavel=(q('orcResponsavel')?.value||'').trim(),aluno=(q('orcAluno')?.value||'').trim(),nasc=q('orcNascimento')?.value||'',idade=ageFromBirth(nasc),observacoes=(q('orcObservacoes')?.value||'').trim(),status=q('orcSaveStatus'),btn=q('saveQuote');
